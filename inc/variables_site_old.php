...
// Fonction prend en paramètre une date de type MySql yyyy-mm-dd hh:mm:ss et la retourne de la manière suivante dd/mm/yyyy hh:mm
...
// Fonction prend en paramètre une date de type MySql yyyy-mm-dd hh:mm:ss et la retourne de la manière suivante dd/mm/yyyy hh:mm:ss
...
// Fonction prend en paramètre une date de type MySql yyyy-mm-dd et la retourne de la manière suivante dd/mm/yyyy
...
// Fonction prend en paramètre une date de type dd/mm/yyyy et la retourne de la manière suivante yyyy-mm-dd
...
// Fonction prend en paramètre une chaine et teste si c'est un entier
...
// Fonction prend en paramètre une chaine et teste si c'est un float
...
echo '<tr class=passage>';
	...
	for ($j = 0; $j < count($ligne); $j++) {
		...
		$tab_valeur_center = array('Derni&egrave;re connexion', 'Date', 'N° Client', 'N° Version', 'Date de Production', 'N° Fournisseur', 'Date création', 'Date modification', 'N° PLV', 'Date commande', 'Statut', 'Date de commande', 'Date de modification', 'Année', 'num_client', 'num_commande', 'num_fournisseur', 'num_plv', 'Date RDV', 'Année - Mois', 'Derniere commande');
		if (in_array($nom_colonne, $tab_valeur_center, true) ) {
			$position='center';
		}
		
		$tab_valeur_right = array('Global','Nb Connexion','Nb prospects','Nb clients', 'Nb fournisseurs', 'Nb commande annulée', 'Nb commande en cours', 'Nb commande terminée', 'Nb devis en cours','Montant HT', 'Nb jours', 'Total HT', 'CA', 'Comm.', 'Quantité', 'Nb Commandes', 'Moy CA', 'Moy Comm.', 'Commission');
...
echo '<tr>';
	...
	for ($j = 0; $j < count($ligne); $j++) {
		...
		$tab_valeur_center = array('Date', 'N° Client', 'N° Fournisseur', 'Date création', 'Date modification', 'N° PLV', 'Date de commande', 'Date de modification', 'Année', 'num_client', 'num_commande', 'num_fournisseur', 'num_plv');
		if (in_array($nom_colonne, $tab_valeur_center, true) ) {
			$position='center';
		}
		
		$tab_valeur_right = array('Global','Total HT', 'CA', 'Comm.', 'Quantité', 'Nb Commandes', 'Moy CA', 'Moy Comm.', 'Commission');
		if (in_array($nom_colonne, $tab_valeur_right, true) ) {
			$position='right';
		}
...
select	num_tiers '$nom_champ1',
		...
		date_format( date_insertion  , '%Y-%m-%d %H:%i' ) 'Date création',
...
$tab = mysql_query($req_sql) or die('<br> Erreur sql f_modification_client');
...
$tab = mysql_query($req_sql) or die('<br> Erreur sql f_modification_fournisseur');
...
select	c.num_tiers		num_client,
        ...
        t.nom_tiers		'Client',
		case c.etat_commande when 'A' then 'Annulée' when 'V' then 'Validée' when 'T' then 'Terminée' when 'E' then 'En cours' else '' end Statut,	
		...
		date_format(c.date_insertion,'%Y-%m-%d')	'Date création'
from	wm_client_commande c,
        wm_ref_tiers       t
where	c.login_site    = '$login'
and		c.login_site    = t.login_site
...
select	c.num_tiers		num_client,
		c.num_commande	num_commande,
		c.num_commande	'$nom_champ',
		case c.etat_commande when 'A' then 'Annulée' when 'T' then 'Terminée' when 'E' then 'En cours' else '' end Statut,	
...
// fonction qui affiche la liste des fournisseurs dans une liste déroulante d'un formulaire -----------------------
...
echo '<td width=5% bgcolor=' . $color_title . ' align=center> Quantité </td>';
...
echo '<tr>';
echo '<td align=center bgcolor='.$color_title.' width=10%> N° PLV </td>';
echo '<td align=center bgcolor='.$color_title.' width=10%> N° Fournisseur </td>';
...
// Headers
$headers = 'From: '.$cpt_nom.' '.$cpt_prenom.' <'.$cpt_e_mail.'>'."\r\n";
...
// Message
$contenu_mail = 'Texte affiché par des clients mail ne supportant pas le type MIME.'."\r\n\r\n";
...
<tr><td>
<br>
Veuillez trouver ci joint le récapitulatif de votre commande n&ordm;$num_commande : <br><br>
</td></tr>
...
// Fonction qui envoie au fournisseur concerné les informations PLV -------------------------------------------------------------
...
if (trim($mail_1_fournisseur) != '') {
	$destinataire='<'.$mail_1_fournisseur.'>';
}
if (trim($mail_2_fournisseur) != '' and trim($mail_1_fournisseur) != '' and trim($mail_1_fournisseur) != trim($mail_2_fournisseur)) {
	$destinataire .= ', <'.$mail_2_fournisseur.'>';
}
if (trim($mail_2_fournisseur) != '' and trim($mail_1_fournisseur) == '') {
	$destinataire .= '<'.$mail_2_fournisseur.'>';
}

if (trim($e_mail_1) != '') { 
	$destinataire .= ', <'.$e_mail_1.'>';
}

if (trim($e_mail_2) != '') { 
	$destinataire .= ', <'.$e_mail_2.'>';
}
...
<tr> 
<td> Date de la livraison : $livraison_date Entre $livraison_h_debut et $livraison_h_fin  </td> 
</tr>
</table>
...
/*
$contenu_mail = str_replace('é','&eacute;', $contenu_mail);
$contenu_mail = str_replace('è','&egrave;', $contenu_mail);
$contenu_mail = str_replace('ê','&ecirc;', $contenu_mail);
$contenu_mail = str_replace('à','&agrave;', $contenu_mail);
$contenu_mail = str_replace('ù','&ugrave;', $contenu_mail);
$contenu_mail = str_replace('ç','&ccedil;', $contenu_mail);
$contenu_mail = str_replace('û','&ucirc;', $contenu_mail);
*/

$headers = str_replace('é','e', $headers);
$headers = str_replace('è','e', $headers);
$headers = str_replace('ê','e', $headers);
$headers = str_replace('à','a', $headers);
$headers = str_replace('ù','u', $headers);
$headers = str_replace('ç','c', $headers);
$headers = str_replace('û','u', $headers);
...
// Fonction qui renvoie la dernière date d'envoi au client pour une commande ---------------------------------------------------
...
if ($flag_client_fournisseur == 'F') {	
	if ($flag_total == 'N') {
		...
		select		c.id_fournisseur																	num_fournisseur,
					...
					date_format(cc.commande_date , '%Y' )												Année, 
					sum(round(c.quantite*c.prix_ht,2))													CA,
					sum(round(c.quantite*c.prix_ht*commission/100, 2))									'Comm.',
					sum(c.quantite)																		'Quantité',
...
if ($flag_client_fournisseur == 'T') { // GLOBAL
	$req_sql = " 
	select		date_format(cc.commande_date , '%Y' )												Année, 
				sum(round(c.quantite*c.prix_ht,2))													CA,
				sum(round(c.quantite*c.prix_ht*commission/100, 2))									'Comm.',
				sum(c.quantite)																		'Quantité',
				count(distinct c.num_commande)														'Nb Commandes',
...
if ($flag_client_fournisseur == 'TM') { // GLOBAL
	$req_sql = " 
	select		date_format(cc.commande_date , '%Y-%m' )												'Année - Mois', 
				sum(round(c.quantite*c.prix_ht,2))													CA,
				sum(round(c.quantite*c.prix_ht*commission/100, 2))									'Comm.',
				sum(c.quantite)																		'Quantité',
				count(distinct c.num_commande)														'Nb Commandes',
...
select	r.login																'Compte',
		concat(r.nom, ' ', r.prenom)										'Nom Prenom', 
 		nom_tiers															'Societe', 
		date_format( r.dat_cre, '%Y-%m-%d' )								'Date création',
...
// fonction qui modifie les dates de début et de fin d'un compte (admin only) ----------------------------------------------------
...
if ($login_ == $login) { // l'utilisateur est connu dans la base, on peut donc lui envoyer un mail
	...
	$_new_passwd_
	<br><br><br>
	Pensez à le changer à votre prochaine connexion.
	<br><br>
	</td>
	</tr></table>";
	...
	*/
	...
	if (mail($destinataire,$sujet,$contenu_mail,$headers)) {
		return "Un mail vient de vous être envoyé.";
	}
	else {
		return "Un problème est survenu lors de l'envoi du mail.";
	}
}
else {
...
// Fonction qui affiche les commandes terminées pour un fournisseur --------------------
...
// fonction qui affiche la liste des fournisseurs - produit dans une liste déroulante d'un formulaire -----------------------
...
// Suppression des clients supprimés par l'utilisateur qui n'a pas de commandes
$req_del_sql ="
delete from wm_ref_tiers 
where login_site    = '$login' 
and   tiers_visible = 'N' 
...
// fonction qui crée un nouveau RDV ----------------------------------------------
...
// fonction qui récupère la date du prochain rdv d'un client ----------------------------------------------
...
// fonction qui récupère la date du dernier rdv d'un client ----------------------------------------------
...
// Fonction qui retourne le n° de client par rapport à un n° de rdv
...
select compte.login 'Compte',
	...
	(select count(distinct num_commande) from wm_client_commande commande where commande.login_site = compte.login and type_commande = 'C' and etat_commande='A') 'Nb commande annulée',
	...
	(select count(distinct num_commande) from wm_client_commande commande where commande.login_site = compte.login and type_commande = 'C' and etat_commande='T') 'Nb commande terminée',
...
select	num_version 'N° Version',
		...
		commentaire	'Evolution apportée'
from	wm_versioning
...
if ($flag_type == 'html') {
$chaine_a_retourner = str_replace('é','&eacute;', $chaine_a_retourner);
$chaine_a_retourner = str_replace('è','&egrave;', $chaine_a_retourner);
$chaine_a_retourner = str_replace('ê','&ecirc;', $chaine_a_retourner);
$chaine_a_retourner = str_replace('à','&agrave;', $chaine_a_retourner);
$chaine_a_retourner = str_replace('ù','&ugrave;', $chaine_a_retourner);
$chaine_a_retourner = str_replace('ç','&ccedil;', $chaine_a_retourner);
$chaine_a_retourner = str_replace('û','&ucirc;', $chaine_a_retourner);
$chaine_a_retourner = str_replace('°','&deg;', $chaine_a_retourner);
}
else {
$chaine_a_retourner = str_replace('é','e', $chaine_a_retourner);
$chaine_a_retourner = str_replace('è','e', $chaine_a_retourner);
$chaine_a_retourner = str_replace('ê','e', $chaine_a_retourner);
$chaine_a_retourner = str_replace('à','a', $chaine_a_retourner);
$chaine_a_retourner = str_replace('ù','u', $chaine_a_retourner);
$chaine_a_retourner = str_replace('ç','c', $chaine_a_retourner);
$chaine_a_retourner = str_replace('û','u', $chaine_a_retourner);
...
// Fonction qui affiche un tableau croisé dynamique par produit en fct° des fournisseurs ou des clients
...
// Fonction qui affiche un tableau croisé dynamique par Client/fournisseur sur la periode en cours
...
// fonction qui exporte les résultats d une requete sous excel  ----------------------------------------------
...
// fonction qui va gérer la req. et l'envoi des données reporting  ----------------------------------------------
...
f_exporte_requete_excel($login, $req_sql, $chemin, $nom_fichier, $mail_objet);
...
// fonction qui exporte les résultats d une requete sous excel  ----------------------------------------------
function f_exporte_requete_excel($login, $req_sql, $chemin, $extract_filename, $mail_objet) {
